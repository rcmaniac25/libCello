...
#elif __QNX__
  with(bps in module("libbps.so")) {
    int (*bps_get_version)(void) = get(bps, $(String, "bps_get_version"));

    println("BPS Version is '%d'", $(Int, bps_get_version()));

    PT_ASSERT(bps_get_version());
  }
#else
